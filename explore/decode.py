...
def speculative_generate(
    ...
    while generated_ids.shape[1] < max_tokens:  # **不再手动检查 max_tokens**
        if not begin:
            ...
            # **更新 token 序列**
            generated_ids = torch.cat([generated_ids, new_ids[:, -1:]], dim=-1)
        ...
        if begin or any(trigger in decoded_text for trigger in TRIGGER_TOKENS):
            ...
            tgt_new_ids, tgt_kv_candidate = generate_with_partial_kv(
                target_model, tokenizer, cache_generated_ids, tgt_kv,
                max_new_tokens=max_target_tokens, temperature=temperature, top_k=top_k, top_p=top_p
            )
            # **解码 Target Model 生成的文本**
            tgt_decoded_text = tokenizer.decode(tgt_new_ids[0], skip_special_tokens=True)
            # **Step 4: 目标模型生成的文本是否包含关键字**
            if begin or contains_keywords(tgt_decoded_text, TARGET_VALIDATION_KEYWORDS):
                correct_tokens.append({'pos': cache_generated_ids.shape[1], 'think':tgt_decoded_text})
                generated_ids = torch.cat([cache_generated_ids, tgt_new_ids[:, :]], dim=-1)  # ✅ 接受 Target Model 结果
...
TARGET_VALIDATION_KEYWORDS = {"wait", "hmm", "alternatively", "okay", 'alright', 'but'}  # 目标关键字
help_think_word = '\n\n'
system_prompt = None
...
for dataset in datasets:
    ...
    for sample in tqdm(dataset.select(range(len(results), len(dataset)))):
        ...
        input_ids = tokenizer.apply_chat_template(messages, return_tensors="pt").input_ids.to(device)
